...
public final class JStyleFrame {
    ...
    public static final int HISTORY_READ_CONTINUATION = 0x02;
    ...
    /** Read-only automatic SpO2 history request; only start and continuation are allowed. */
    public static byte[] autoSpO2HistoryRequest(int mode) {
        if (mode != HISTORY_READ_START && mode != HISTORY_READ_CONTINUATION) {
            throw new IllegalArgumentException("Auto SpO2 history mode must be read start or continuation");
        }
        return readRequest(AUTO_SPO2_HISTORY_COMMAND, mode);
    }

    public static boolean isReadOnlyDiagnosticRequest(byte[] frame) {
        return java.util.Arrays.equals(frame, versionReadRequest())
                || java.util.Arrays.equals(frame, autoSpO2ConfigReadRequest())
                || java.util.Arrays.equals(frame, autoSpO2HistoryRequest(HISTORY_READ_START))
                || java.util.Arrays.equals(frame, autoSpO2HistoryRequest(HISTORY_READ_CONTINUATION));
    }
    ...
    /** JStyle/2208 measurement response stores SpO2 at byte index 3. */
    public static int spO2Percent(byte[] frame) {
    ...
    /** Returns the newest valid value in a history packet, or 0 when no value exists. */
    public static int manualSpO2Percent(byte[] packet) {
    ...
    public static boolean isAutoSpO2HistoryRecord(byte[] packet) {
        return packet != null && packet.length == 10
    ...
    public static AutoSpO2Record parseAutoSpO2Record(byte[] packet) {
        if (!isAutoSpO2HistoryRecord(packet)) {
            throw new IllegalArgumentException("Auto SpO2 history record must be exactly 10 bytes");
        }
        int percent = packet[9] & 0xFF;
        if (percent < 1 || percent > 100) {
            throw new IllegalArgumentException("Auto SpO2 percent must be 1..100");
        }
        int year = 2000 + bcd(packet[3], "year");
        int month = bcd(packet[4], "month");
        int day = bcd(packet[5], "day");
        int hour = bcd(packet[6], "hour");
        int minute = bcd(packet[7], "minute");
        int second = bcd(packet[8], "second");
        try {
            return new AutoSpO2Record(packet[1] & 0xFF, percent,
